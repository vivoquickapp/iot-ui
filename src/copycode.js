...
function fileDisplay(filePath, arr) {
  ...
  files.forEach(function(filename) {
    ...
    if (isFile) {
      if (
        !filedir.endsWith(".DS_Store") &&
        !filedir.endsWith(".png") &&
        !filedir.endsWith(".jpg") &&
        !filedir.endsWith(".jpeg")
      ) {
        arr.push(filedir);
      }
    }
...
arr.map(item => {
  fs.appendFileSync(
    path.join(__dirname, "代码.txt"),
    item.replace("/Users/luopingyi/Desktop/iot-ui/src/iotUI/", "") + "\r"
  );
